    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            result = await run_workflow(
                db, run.id, prompt,
                agent_ids=None,          # use all active agents (orchestrator picks up)
                session_key=f"scheduled_{agent_id}",
            )
            run_obj = await db.get(__import__("app.models.workflow_run", fromlist=["WorkflowRun"]).WorkflowRun, run.id)
            if run_obj:
...
def register_agent_job(agent_id: int, cron: str, prompt: str) -> None:
    ...
    try:
        ...
        logger.info("Scheduled job registered: agent_id=%s cron='%s'", agent_id, cron)
    except Exception as exc:
